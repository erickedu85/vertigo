 ...
 * type {@link EdgeMst} and has a real-valued weight. It supports the following two
...
public class EdgeWeightedGraph {
	...
	public EdgeWeightedGraph(int V) {
		...
		this.V = V;
		this.E = 0;
		adj = (Bag<EdgeMst>[]) new Bag[V];
		for (int v = 0; v < V; v++) {
			adj[v] = new Bag<EdgeMst>();
		}
	}
	...
	public EdgeWeightedGraph(int V, int E) {
		...
		for (int i = 0; i < E; i++) {
			...
			EdgeMst e = new EdgeMst(v, w, weight);
			addEdge(e);
		}
	}
	...
	public EdgeWeightedGraph(EdgeWeightedGraph G) {
		...
		for (int v = 0; v < G.V(); v++) {
			...
			Stack<EdgeMst> reverse = new Stack<EdgeMst>();
			for (EdgeMst e : G.adj[v]) {
				reverse.push(e);
			}
			for (EdgeMst e : reverse) {
				adj[v].add(e);
			}
		}
	}
	 ...
	 *             unless {@code 0 <= v < V}
	 */
	public Iterable<EdgeMst> adj(int v) {
		validateVertex(v);
		return adj[v];
	}
	...
	public Iterable<EdgeMst> edges() {
		Bag<EdgeMst> list = new Bag<EdgeMst>();
		for (int v = 0; v < V; v++) {
			int selfLoops = 0;
			for (EdgeMst e : adj(v)) {
				if (e.other(v) > v) {
