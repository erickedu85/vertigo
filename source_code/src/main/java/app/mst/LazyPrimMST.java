 ...
 * connected component. The {@code weight()} method returns the weight of a
 * minimum spanning tree and the {@code edges()} method returns its edges.
 * <p>
 ...
 * alternate implementations, see {@link PrimMST}, {@link KruskalMST}, and
 * {@link BoruvkaMST}.
 *
...
public class LazyPrimMST {
	...
	private Queue<EdgeMst> mst; // edges in the MST
	private boolean[] marked; // marked[v] = true if v on tree
	private MinPQ<EdgeMst> pq; // edges with one endpoint in tree
	...
	public LazyPrimMST(EdgeWeightedGraph G) {
		mst = new Queue<EdgeMst>();
		pq = new MinPQ<EdgeMst>();
	...
	private void prim(EdgeWeightedGraph G, int s) {
		...
		while (!pq.isEmpty()) { // better to stop when mst has V-1 edges
			EdgeMst e = pq.delMin(); // smallest edge on pq
...
//		In in = new In(args[0]);
		EdgeWeightedGraph G = new EdgeWeightedGraph(6);//6 vertices
		G.addEdge(new EdgeMst(1, 2, 3.0));
		G.addEdge(new EdgeMst(1, 3, 6.0));
		G.addEdge(new EdgeMst(1, 4, 5.0));
		G.addEdge(new EdgeMst(1, 5, 1.0));
		G.addEdge(new EdgeMst(2, 3, 2.0));
		G.addEdge(new EdgeMst(2, 4, 4.0));
		G.addEdge(new EdgeMst(2, 5, 7.0));
		G.addEdge(new EdgeMst(5, 2, 7.0));
		G.addEdge(new EdgeMst(3, 4, 8.0));
		G.addEdge(new EdgeMst(3, 5, 9.0));
		G.addEdge(new EdgeMst(4, 5, 16.0));
